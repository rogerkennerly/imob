...
<?php
	...
	if($tipo_destaque == 1){
		...
		$s_destaque = mysql_query("SELECT * FROM imovel WHERE disponivel = 'S' AND excluido = 'N' AND id_finalidade = 1 AND destaque = 'S'");
	...
	if($tipo_destaque == 2){
		...
		$s_destaque = mysql_query("SELECT * FROM imovel WHERE disponivel = 'S' AND excluido = 'N' AND id_finalidade = 2 AND destaque = 'S'");
	...
	if($tipo_destaque == 3){
		...
		$valor_min = $imovel['valor'] - ($imovel['valor'] * 10 / 100);
		$valor_max = $imovel['valor'] + ($imovel['valor'] * 10 / 100);

		$s_destaque = mysql_query("SELECT * FROM imovel WHERE disponivel = 'S' AND excluido = 'N' AND id <> ".$imovel['id']." AND id_cidade = ".$imovel['id_cidade']." AND valor >= '$valor_min' AND valor <= '$valor_max'");
	...
	if($tipo_destaque == 4){
		...
		$s_destaque	= mysql_query("SELECT * FROM imovel WHERE disponivel = 'S' AND excluido = 'N' AND id <> ".$imovel['id']." AND ($q_complemento)");
	}
	...
	if($exibir_destaque){?>
					...
					while($destaque = mysql_fetch_assoc($s_destaque)){?>
						<div class="imob_destaques_pequeno_imoveis_item">
							 <?php  
		                        ...
		                        $q_foto      = mysql_query("SELECT * FROM foto WHERE id_imovel = ".$destaque['id']." LIMIT 0,1");
		                        $foto_imovel = mysql_fetch_assoc($q_foto);
		                    ?>
							<div class="imob_destaques_pequeno_imoveis_item_img">
								<a href="index.php?pg=imovel&id=<?php echo $destaque['id']; ?>" title="<?php echo $title; ?>">
									<?php  
		                                if(mysql_num_rows($q_foto) > 0){?>
		                                    <img src="fotos/<?php echo $destaque['id']; ?>/t2/<?php echo $foto_imovel['nome']; ?>"><?php
		                                }else{?>
		                                    <img src="fotos/nenhuma_foto.jpg"><?php
		                                }
		                            ?>

									<div class="imob_destaques_pequeno_imoveis_item_img_infos">
										<div class="imob_destaques_pequeno_imoveis_item_img_infos_local">
											<h1><?php echo retorna_nome('bairro', $destaque['id_bairro']); ?>, <?php echo retorna_nome('cidade', $destaque['id_cidade']); ?></h1>
		                                    <span><?php echo retorna_nome('tipo', $destaque['id_tipo']); ?> | <?php echo retorna_nome('finalidade', $destaque['id_finalidade']); ?></span>
										</div>
										<div class="imob_destaques_pequeno_imoveis_item_img_infos_preco">
		                                    <?php  
		                                        if($destaque['valor']){?>
		                                             <h1>R$ <?php echo tratar_moeda($destaque['valor'], 2); ?></h1><?php
		                                        }
		                                        else{?>
										...
										<div class="imob_destaques_pequeno_imoveis_item_img_infos_ref">
                                          <span><?php echo "Ref. ".$destaque['ref']; ?></span>
                                        </div>
                                        <div class="hack"></div>
										<div class="imob_destaques_pequeno_imoveis_item_img_infos_especs">
											<ul class="imob_destaques_pequeno_imoveis_item_img_infos_especs_1">
												<?php
						                        	if($destaque['quarto']){?>
		...
		<?php
          $num_itens = 0; 
          if($config['destaques_pequeno'] == 0){
            $num_itens = 3;
          }
          else{
            $num_itens = $config['destaques_pequeno'];
          }
        ?>
