...
<?php
	...
	$s_cidade = "SELECT nome FROM cidade WHERE id = '".$dados["id_cidade"]."'";
	$q_cidade = mysql_query($s_cidade);
	$cidade = mysql_fetch_assoc($q_cidade);
			...
			<?php  
				...
				if($dados['endereco']){?>
					<div class="imob_contato_informacoes_item">
						...
						<span> <?php echo $dados['endereco']. '  ' .$dados['bairro']. ' - ' .$cidade['nome']. '/' .$dados['estado']; ?></span>
					</div><?php
				}
			?>
			<?php
				...
				if($latitude == '' OR $longitude == ''){
					$endereco = $dados['endereco']." ".$dados['bairro']." - ".$cidade['nome'].", ".$dados['estado'];
