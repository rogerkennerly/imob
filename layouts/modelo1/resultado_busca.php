...
<?php
	...
	if($referencia){
		$st = $s." AND ref = '$referencia'";
		$tipo_query = "";
		$qtesteref = mysql_query($st);
		if(mysql_num_rows($qtesteref)>0){
   ...
   <?php  
      if($_GET['layout'] == 'lista'){
         ...
         $classe_layout = 'imob_resultado_busca_bloco';
			...
			$classe_layout = "imob_resultado_busca_".$_SESSION['tipo_layout'];
      }
	?>
