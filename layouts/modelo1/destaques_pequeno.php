...
<?php
	...
	if($tipo_destaque == 1){
		...
		$s_destaque = mysql_query("SELECT imovel.*, imovel_finalidade.id_finalidade, imovel_finalidade.valor as valor_finalidade FROM imovel, imovel_finalidade WHERE disponivel = 'S' AND excluido = 'N' AND imovel_finalidade.id_finalidade = 1 AND destaque = 'S' AND imovel.id = imovel_finalidade.id_imovel ORDER BY RAND() LIMIT 0,6");

		if(mysql_num_rows($s_destaque) < 3){ 
      $s_destaque = mysql_query("SELECT DISTINCT(imovel.id), imovel.*, imovel_finalidade.id_finalidade, imovel_finalidade.valor as valor_finalidade FROM imovel, imovel_finalidade, foto WHERE disponivel = 'S' AND excluido = 'N' AND imovel_finalidade.id_finalidade = 1 AND imovel.id = imovel_finalidade.id_imovel AND foto.id_imovel = imovel.id ORDER BY RAND() LIMIT 0,6");
		}
	...
	if($tipo_destaque == 2){
		...
		$s_destaque = mysql_query("SELECT imovel.*, imovel_finalidade.id_finalidade, imovel_finalidade.valor as valor_finalidade FROM imovel, imovel_finalidade WHERE disponivel = 'S' AND excluido = 'N' AND imovel_finalidade.id_finalidade = 2 AND destaque = 'S' AND imovel.id = imovel_finalidade.id_imovel ORDER BY RAND() LIMIT 0,6");

		if(mysql_num_rows($s_destaque) < 3){ 
      $s_destaque = mysql_query("SELECT DISTINCT(imovel.id), imovel.*, imovel_finalidade.id_finalidade, imovel_finalidade.valor as valor_finalidade FROM imovel, imovel_finalidade, foto WHERE disponivel = 'S' AND excluido = 'N' AND imovel_finalidade.id_finalidade = 2 AND imovel.id = imovel_finalidade.id_imovel AND foto.id_imovel = imovel.id ORDER BY RAND() LIMIT 0,6");
		}
	...
	if($tipo_destaque == 4){
		...
		for($i=0; $i<count($_SESSION['ultimos_visitados']); $i++){
			if($imovel['id'] != $_SESSION['ultimos_visitados'][$i]){
				...
				$q_complemento .= " imovel.id = ".$_SESSION['ultimos_visitados'][$i];
							...
							<a href="/imovel/<?php echo $destaque['id']; ?>/<?php echo $nome_fin; ?>">
								...
								<div class="imob_destaques_pequeno_imoveis_item_infos">
									...
									<div class="imob_destaques_pequeno_imoveis_item_infos_preco">
										<?php  
											if($destaque['valor_finalidade'] > 0){?>
