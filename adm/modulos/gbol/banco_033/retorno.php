...
<?php
  ...
  foreach ($conteudo_arquivo as $linha)
  {
    ...
    {       
      ...
      if ( $tipo_segmento == 'T' ) // echo "Tratamento do segmento T\n";
      {
        ...
        $titulo[$id_titulo]['data_vencto']                    = substr($linha,73,4).'/'.substr($linha,71,2).'/'.substr($linha,69,2);
        ...
        $titulo[$id_titulo]['pagador_nome']                   = trim(substr($linha, 143, 40));
        ...
        if ( $titulo[$id_titulo]['carteira'] == '2' ) //sem registro
        { $titulo[$id_titulo]['nosso_numero'] = substr($titulo[$id_titulo]['nosso_numero'],0,-1);
          if ($titulo[$id_titulo]['pagador_nome'] == '') { $titulo[$id_titulo]['pagador_nome'] = 'TITULO SEM REGISTRO'; }
        }
        /*
      ...
      {       
        ...
        $titulo[$id_titulo]['data_ocorrencia']                = substr($linha,141,4).'/'.substr($linha,139,2).'/'.substr($linha,137,2);
        $titulo[$id_titulo]['data_credito']                   = substr($linha,149,4).'/'.substr($linha,147,2).'/'.substr($linha,145,2);
  ...
  { 
    ...
    $titulo[$t]['nota'] = codigomovimento($titulo[$t]['cod_movimento']);
    ...
    { foreach($temp_rejeicao as $cod_ocorrencia)
      { $titulo[$t]['nota'] .= rejeicoes($cod_ocorrencia);
      }
      $titulos_recusados[]  = $titulo[$t];
      $titulo[$t]['situacao'] = 'Recusado';
    }
    ...
    { foreach($temp_rejeicao as $cod_ocorrencia)
      ...
      if ( $titulo[$t]['cod_movimento']=='06' || $titulo[$t]['cod_movimento']=='17' ) { $titulos_liquidados[] = $titulo[$t]; $titulo[$t]['situacao'] = 'Liquidado'; }
    }
    elseif ( $titulo[$t]['cod_movimento'] != '02')
    { foreach($temp_rejeicao as $cod_ocorrencia)
      { $s = codigomovimento($titulo[$t]['cod_movimento']);
        if ($s != '') { $titulo[$t]['nota'] .= $s; $s = ''; } else { $s = codigoocorrencia($cod_ocorrencia); }
        if ($s != '') { $titulo[$t]['nota'] .= $s; $s = ''; }
      }
      $titulos_outros[] = $titulo[$t];
      $titulo[$t]['situacao'] = 'Outro';
    }
    if ( $titulo[$t]['cod_movimento']=='02' ) { $titulos_aceitos[] = $titulo[$t];  $titulo[$t]['situacao'] = 'Aceito'; }
    ...
    //echo 'numero=',$titulo[$t]['nosso_numero'],' | tarifa=',$titulo[$t]['valor_tarifa'],' | nota=',$titulo[$t]['nota'],'<br>';
  }
