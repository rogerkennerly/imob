...
<?php
    echo "<table>\n";
    echo "<tr><td align=center colspan=2><b>Crediconai/Bancoob não tá certo isso ainda</td></tr>\n";
...
<script language="JavaScript">
  function check_formaspagto(f){
    ...
    s = trim(f.elements["ag_001"      ].value); if (s.length != 4) { alert("Agência deve ter 4 caracteres"               ); return false; }
    s = trim(f.elements["digag_001"   ].value); if (s.length != 1) { alert("Dígito da Agência deve ter 1 caracter"       ); return false; }
    s = trim(f.elements["cc_001"      ].value); if (s.length != 8) { alert("Conta Corrente deve ter 8 caracteres"        ); return false; }
    s = trim(f.elements["digcc_001"   ].value); if (s.length != 1) { alert("Dígito da Conta Corrente deve ter 1 caracter"); return false; }
