    ...
    if($q = mysql_query($s)){
      ...
      if($senha){
        if($senha == $confirma_senha){
          mysql_query("UPDATE usuario SET senha = '$senha' WHERE id = '$id_usuario'");
          
          echo sucesso("Senha alterada com sucesso!");
        }
        else{
          echo alerta("As senhas não coincidem");
        }
      }
    }else{
    ...
    <div class="col-xs-12">
      <form method="POST" action="" enctype="multipart/form-data" id="form_cadastro">
        ...
        <div class="imob-50 imob-left imob-padding-20">
          ...
          $tipo = listar("tipo");?>
          <select name="tipo">
            <option value="1" <?php if($r["tipo"] == 1){echo "selected";} ?>>Corretor</option>
            <option value="2" <?php if($r["tipo"] == 2){echo "selected";} ?>>Editor</option>
            <option value="3" <?php if($r["tipo"] == 3){echo "selected";} ?>>Administrador</option>
          </select>
