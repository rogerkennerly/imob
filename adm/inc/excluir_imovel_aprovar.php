<?php
  ...
  if(mysql_num_rows($qt)>0){
    $s = "DELETE FROM cadastro_imovel WHERE id = '$id_mensagem'";
    if(mysql_query($s)){
      $_SESSION["session_sucesso"] = "Imóvel para aprovação $id_mensagem excluído com sucesso!";
      
      $descricao_log = "Imóvel para aprovação - id. $id_mensagem";
  ...
  header("Location: ../index.php?pg=aprovar-imoveis");
?>