<?php
	$s_npermissoes = "SELECT id FROM modulo_item WHERE id in (SELECT id_modulo_item FROM permissao WHERE id_usuario = '".$_SESSION["sessao_id_user"]."' AND (id_modulo_item = 45 OR id_modulo_item = 47 OR id_modulo_item = 48))";
	// echo $s_npermissoes;
	$q_npermissoes = mysql_query($s_npermissoes);
	$acoes_permitidas = mysql_num_rows($q_npermissoes);
	$permissoesz = array();
	while($r_npermissoes = mysql_fetch_assoc($q_npermissoes)){
		$permissoesz[] = $r_npermissoes['id'];
	}
	// echo "<pre>";var_dump($permissoesz);echo "</pre>";
	$permissao_fotos = mysql_query("SELECT id FROM permissao WHERE id_usuario = '".$_SESSION["sessao_id_user"]."' AND id_modulo_item = '25'");
	if(mysql_num_rows($permissao_fotos)>0){
		$permissao_fotos = 1;
	}
	else{
		$permissao_fotos = 0;
        ...
        <table id="sample-table-1" class="table table-striped table-bordered table-hover">
          <thead>
            <tr>
              ...
              <th colspan="<?php echo $acoes_permitidas; ?>" class="center">OPÇÕES</th>
            </tr>
          </thead>
                ...
                <tr>
                  ...
                  <td class="center">
										...
										if($config['restricao_proprietarios'] == 'S' AND $_SESSION["sessao_tipo_user"] == 1){
											if($r["id_corretor"] != $_SESSION['sessao_id_user']){
												$mostrar_proprietarios = 0;
											}
										}
										if($mostrar_proprietarios == 1){
											echo retorna_nome("proprietario", $r["id_proprietario"]); 
										}
										else{
											echo "-";
										}
                  ...
                  <td class="center"><?php echo tratar_data_hora($r["data_cadastro"], 2); ?></td>
                  ...
                  <td class="center" width="30">
									<?php
									if(in_array('45', $permissoesz)){?>
                    <div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                      <a href="?pg=ver-dados-imovel&id_imovel=<?php echo $r["id"]; ?>" style="display:block;width:2.5rem;">
