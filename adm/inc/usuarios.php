<?php
  ...
  if($_POST["op"] == "alterar"){
    ...
    $msg = sucesso("Usuario Alterada com sucesso!");
  }
  elseif($_POST){
    $nome = $_POST["nome"];
    if(isset($nome)){
      ...
      if(mysql_num_rows($teste)>0){
        $msg = alerta("Usuario $nome já existe!");
      }else{
        mysql_query("INSERT INTO $tabela (nome) VALUES ('$nome')");
        $msg = sucesso("Usuario $nome cadastrado com sucesso!");
      }
    }
  }
  ...
  if($_GET["op"] == "excluir"){
    ...
    $msg = sucesso("Usuário excluido com sucesso!");
    
    $descricao_log = "Usuário excluido - $nome ($usuario)";
      ...
      <table id="sample-table-1" class="table table-striped table-bordered table-hover">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="center">
              <a href="index.php?pg=editar-usuario&id=<?php echo $r["id"]; ?>" style="display:block;float:left;width:2.5rem;margin-right:1rem;">
                <button class="btn btn-xs btn-info" style="margin-right:10px;width:2.5rem;">
                  <i class="icon-edit bigger-120"></i>
                </button>
              </a>
              
              <a href="index.php?pg=usuarios&op=excluir&id_registro=<?php echo $r["id"]; ?>"onclick="return confirm('Deseja excluir o registro?')" style="display:block;float:left;width:2.5rem;">
